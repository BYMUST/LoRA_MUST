...
def get_backbone(args, pretrained=False):
    ...
    # VPT
        ...
        if args["model_name"] == "adam_lora" :
            from backbone import vision_transformer_lora
            from easydict import EasyDict
            tuning_config = EasyDict(
                # LoRAFormer
                ffn_lora=True,
                ffn_option="parallel",
                # ffn_lora_layernorm_option="none",
                # ffn_lora_init_option="lora",
                # ffn_lora_scalar="0.1",
                lora_dim = args['lora_dim'],
                lora_scaling = args['lora_scaling'],
                lora_droppout = args['lora_droppout'],
                d_model=768,
                # VPT related
                vpt_on=False,
                vpt_num=0,
            ...
            if name == "pretrained_vit_b16_224_lora":
                model = vision_transformer_lora.vit_base_patch16_224_lora(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
...
class MultiBranchCosineIncrementalNet(BaseNet):
    ...
    def construct_dual_branch_network(self, tuned_model):
        ...
        elif 'lora' in self.args['backbone_type']:
            newargs=copy.deepcopy(self.args)
            newargs['backbone_type']=newargs['backbone_type'].replace('_lora','')
            print(newargs['backbone_type'])
            self.backbones.append(get_backbone(newargs)) #pretrained model without adapter
        else:
